# Hola, soy Andrea Gomez 👋

![AWS](https://img.shields.io/badge/AWS-%23FF9900.svg?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/kubernetes-%23326ce5.svg?style=for-the-badge&logo=kubernetes&logoColor=white)
![IaC](https://img.shields.io/badge/IaC-CloudFormation%20|%20CDK-blue?style=for-the-badge&logo=amazonaws)

## ☁️ Cloud & AWS Solutions Architect
Soy una profesional enfocada en la infraestructura escalable, automatización y seguridad en la nube. En este perfil documento mi ruta de aprendizaje y la implementación de soluciones reales en **AWS**, desde la gobernanza inicial hasta la orquestación de microservicios.

---

### 🚀 Mis Proyectos Destacados (AWS Labs)

#### 🏗️ Infraestructura y Redes
* **Networking Avanzado:** Configuración de VPCs, conectividad híbrida y seguridad de red.
* **IaC:** Automatización de infraestructura completa mediante **CloudFormation** y **AWS CDK**.

#### ⚡ Arquitecturas Serverless y Automatización
* **Event-Driven Design:** Flujos automatizados con **Lambda, API Gateway y EventBridge**.
* **Orquestación:** Gestión de procesos complejos con **AWS Step Functions**.

#### 🐳 Contenedores y DevOps
* **Orquestación:** Gestión de clústeres productivos en **Amazon EKS (Kubernetes)**.
* **CI/CD:** Pipelines de despliegue continuo con **GitHub Actions**.

#### 🔐 Seguridad, Gobernanza y FinOps
* **Compliance:** Implementación de **SCPs** (Service Control Policies) y monitoreo con **CloudTrail**.
* **Costos:** Gestión y optimización de presupuestos con **AWS Cost Explorer**.

---

### 🛠️ Herramientas y Skills
- **Cloud:** AWS (VPC, S3, IAM, Lambda, DynamoDB, EKS, CloudWatch).
- **Herramientas:** Docker, Git, GitHub Actions, CloudFormation, CDK.
- **Background:** Especialista en Electrónica y Tecnología de la Información.

---

📫 **¿Quieres contactarme?** [www.linkedin.com/in/yennyferg-cloud]
